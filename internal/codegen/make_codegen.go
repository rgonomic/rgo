...
func MakevarsTemplate() *template.Template {
	return template.Must(template.New("Makevars").Funcs(template.FuncMap{
		"module": mod.Module,
...
go:
	rm -f *.h
	CGO_CFLAGS=$(CGO_CFLAGS) CGO_LDFLAGS=$(CGO_LDFLAGS) go pack -o $(SHLIB) -packmode=c-shared {{$m := module "."}}{{$m.Path}}/src/rgo/{{.Pkg.Name}}
`))
}
